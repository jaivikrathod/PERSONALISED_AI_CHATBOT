...
export default function ChatbotsPage() {
  ...
  const confirmDelete = async () => {
    ...
    const chatbotId = deleting.id ?? deleting.uuid

    if (!chatbotId) {
      dispatch(
        addToast({
          type: 'error',
          message: 'Unable to determine chatbot ID.',
        })
      )
      return
    }
    ...
    const result = await dispatch(deleteChatbot(chatbotId))
  ...
  return (
    <div>
      <PageHeader
        title="Chatbots"
        subtitle="Create and deploy AI chatbots across your channels."
      >
        <Button
          onClick={() => {
            setEditing(null)
      ...
      {status === 'loading' ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {bots.map((bot) => {
            const chatbotId = bot.id ?? bot.uuid

            return (
              <div
                key={chatbotId}
                className="card-base flex flex-col p-5 transition hover:shadow-card-hover"
              >
                ...
                <div className="mt-1 flex items-center gap-2">
                  <Badge tone={STATUS_TONE[bot.status] || 'gray'} dot>
                    {bot.status || 'Unknown'}
                  </Badge>
      ...
      <ConfirmDialog
        ...
        description={
          deleting
            ? `Delete "${deleting.name}"? The widget will stop working immediately.`
            : ''
        }
        confirmLabel="Delete"
      />
